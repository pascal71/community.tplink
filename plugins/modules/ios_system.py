...
DOCUMENTATION = """
module: ios_system
author: Kjell & Pascal van Dam
short_description: Manage the system attributes on TP-Link business switches
description:
- This module provides declarative management of node system attributes on TP-Link 
  business switches.  It provides an option to configure host system parameters or remove those
  parameters from the device active configuration.
version_added: 0.4.0 
extends_documentation_fragment:
- community.tplink.ios
notes:
...
options:
  ...
  contact:
    decription:
    - Provides the contact-info for this switch. This option takes an ASCII string value.
    type: str
  location:
    decription:
    - Provides the location-info for this switch. This option takes an ASCII string value.
    type: str
  led:
    description:
    - Specifies wether or not the link-activity LEDs are switched on (True) or off (False).
      Default settings is LEDs are on (True)
    type: bool
  domain_name:
    description:
...
from ansible_collections.community.tplink.plugins.module_utils.network.ios.ios import (
    get_config,
    load_config,
)
from ansible_collections.community.tplink.plugins.module_utils.network.ios.ios import (
...
def map_obj_to_commands(want, have, module):
    ...
    if state == "absent":
        ...
        # Added for TP-Link switches

        if have["contact"]:
            commands.append("no contact-info")

        if have["location"]:
            commands.append("no location")

        if have["led"] is False:
            commands.append("led off")

        if have["ntp"]:
            commands.append("no system-time ntp")

        if have["dst"]:
            commands.append("no system-time dst")

        if have["controller_enabled"] is False:
            commands.append("no controller cloud-based")

        if have["inform_url"]:
            commands.append("no controller inform-url")

        if have["log_host"]:
            commands.append("no logging host index")
        ...
        if needs_update("contact"):
            commands.append("contact-info \"%s\"" % want["contact"])

        if needs_update("location"):
            commands.append("location \"%s\"" % want["location"])

        if needs_update("led"):
            if want["led"] is False:
                cmd = "led off"
            else:
                cmd = "led on"
            commands.append(cmd)

        if needs_update("controller_enabled"):
            if want["controller_enabled"] is False:
                cmd = "controller cloud-based"
            else:
                cmd = "no controller cloud-based"


        if needs_update("inform_url"):
            commands.append("controller inform-url \"%s\"" % want["inform_url"])

        if needs_update("dst"):
            commands.append("system-time dst predefined \"%s\"" % want["dst"])

        if needs_update("ntp"):
            commands.append("system-time ntp UTC+01:00 %s 10.8.58.111 11" % want["ntp"])

        if needs_update("log_host"):
            commands.append("logging host index 1 %s 6" % want["log_host"])
...
def parse_contact(config):
    match = re.search("^contact (\\S+)", config, re.M)
    if not match:
      return None
    else:
      return match.group(1)
...
def parse_location(config):
    match = re.search("^location (\\S+)", config, re.M)
    if not match:
      return None
    else:
      return match.group(1)
...
def parse_url(config):
    match = re.search("^controller inform_url (\\S+)", config, re.M)
    if not match:
      return None
    else:
      return match.group(1)

def parse_ntpconf(config):
    match = re.search("^system-time ntp UTC+01:00 (\\S+)", config, re.M)
    if not match:
      return None
    else:
      return match.group(1)

def parse_logconf(config):
    match = re.search("^logging host index 1 (\\S+)", config, re.M)
    if not match:
      return None
    else:
      return match.group(1)

def parse_dstconf(config):
    match = re.search("^system-time dst predefined (\\S+)", config, re.M)
    if not match:
      return None
    else:
      return match.group(1)
...
# Added contact and location for TP-Link

def map_config_to_obj(module):
    config = get_config(module)
    return {
        "hostname": parse_hostname(config),
        "contact": parse_contact(config),
        "location": parse_location(config),
        "led": "led off" not in config,
        "controller_enabled": "controller cloud-based" in config,
        "inform_url": parse_url(config),
        "ntp": parse_ntpconf(config),
        "dst": parse_dstconf(config),
        "log_host": parse_logconf(config),
...
def map_params_to_obj(module):
    obj = {
        ...
        "inform_url": module.params["inform_url"],
        "dst": module.params["dst"],
        "ntp": module.params["ntp"],
        "led": module.params["led"],
        "log_host": module.params["log_host"],
        "controller_enabled": module.params["controller_enabled"],
...
def main():
    ...
    argument_spec = dict(
        ...
        log_host=dict(),
        led=dict(type="bool"),
        controller_enabled=dict(type="bool"),
    ...
    if commands:
        out.write('Let me do it\n') 
        if not module.check_mode:
            out.write('Let me do it really\n') 
