...
description: Kjell & Pascal van Dam
    ...
    suboptions:
      ...
      privilege:
        ...
        - The C(privilege) argument configures the privilege level of the user when logged
          into the system. This argument accepts power_user, operator, admin or user.
        type: int
      view:
        description:
...
- name: set multiple users to privilege level power_user
  community.tplink.ios_user:
    aggregate:
    - name: netop
    - name: netend
...
def validate_privilege(value, module):
    if value not in ["user","power_user","operator","admin"]:
        module.fail_json(
            msg="privilege level should be one of user, power_user, operator or admin, got %s" % value
        )


def user_del_cmd(username):
    return {
        "command": "no user name %s" % username,
...
def map_obj_to_commands(updates, module):
    ...
    def add(command, want, x):
        command.append("user name %s %s" % (want["name"], x))

    def add_hashed_password(command, want, x):
        command.append(
            "user name %s secret %s %s"
    ...
    def add_ssh(command, want, x=None):
        ...
        if x:
            command.append("user name %s" % want["name"])
            for item in x:
                command.append("key-hash %s" % item)
            command.append("exit")
        else:
            command.append("no user name %s" % want["name"])
        command.append("exit")
...
def map_config_to_obj(module):
    data = get_config(module, flags=None)
    match = re.findall("(?:^(?:u|\\s{2}u))ser name (\\S+)", data, re.M)
    if not match:
        return list()
    instances = list()
    for user in set(match):
        regex = "user name %s .+$" % user
