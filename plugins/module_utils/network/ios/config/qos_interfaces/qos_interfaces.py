...
class Qos_Interfaces(ConfigBase):
    ...
    gather_network_resources = ["qos_interfaces"]

    def __init__(self, module):
        super(Qos_Interfaces, self).__init__(module)

    def get_qos_interfaces_facts(self, data=None):
        ...
        qos_interfaces_facts = facts["ansible_network_resources"].get(
            "qos_interfaces"
        )
    ...
    def execute_module(self):
        ...
        if self.state in self.ACTION_STATES:
            ...
            existing_qos_interfaces_facts = []
        if self.state in self.ACTION_STATES or self.state == "rendered":
            commands.extend(self.set_config(existing_qos_interfaces_facts))
        ...
        if self.state in self.ACTION_STATES or self.state == "gathered":
            ...
            result["parsed"] = self.get_qos_interfaces_facts(
                data=running_config
            )
        else:
            changed_qos_interfaces_facts = []
        ...
        if self.state in self.ACTION_STATES:
            ...
            result["gathered"] = changed_qos_interfaces_facts
    ...
    def _set_config(self, want, have):
        ...
        if diff:
            ...
            if trust_mode:
                cmd = "qos trust mode {0}".format(trust_mode)
                add_command_to_config_list(interface, cmd, commands)
