...
class Lag_interfaces(ConfigBase):
    ...
    def execute_module(self):
        ...
        commands = list()
        warnings = list()
        of = open("/tmp/lag-exec.txt","a")
        of.write ("exec of config\n")
    ...
    def set_config(self, existing_lag_interfaces_facts):
        ...
        of = open("/tmp/lag_set_config.log", "a") 
        of.write("set_config\n")
        ...
        if config:
            for each in config:
                ...
                of.write('\n')
                of.write("normalized: ")
                of.write(normalize_interface(each["name"]))
                of.write('\n')
    ...
    def _state_merged(self, want, have, module):
        ...
        of = open("/tmp/lag_merge.log","a")
        of.write("_state_merged\n")
        ...
        #j = json.dumps(want, indent=4)
        #of.write(j)
        #j = json.dumps(have, indent=4)
        #of.write(j)
        #j = json.dumps(module, indent=4)
        #of.write(j)

        for interface in want:
            for each_interface in interface.get("members"):
                of.write("Interface in want: %s\n" % interface.get("name"))
                of.write("   Interface each: %s\n" % each_interface.get("name"))
                ...
                for each in have:
                    if each.get("members"):
                        ...
                        for every in each.get("members"):
                            of.write ("every %s\n" % every["member"])
                            of.write ("each_interface %s\n" % each_interface["member"])
                            if every["member"] == each_interface["member"]:
                                break
                    elif each.get("name") == each_interface["member"]:
                        break
    ...
    def _set_config(self, want, have, module):
        ...
        of = open("/tmp/lag-config2.txt","a")
        of.write ("_set_config\n")
        of.write ("config of lag\n")
