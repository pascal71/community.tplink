...
class Interfaces(ConfigBase):
    ...
    params = ("description", "mtu", "speed", "duplex", "eee", "poe", "flow-control")
    ...
    def execute_module(self):
        ...
        out = open ("/tmp/exec.log","a") 
        out.write ("Here we go... \n")

        if commands and self.state in self.ACTION_STATES:
            if not self._module.check_mode:
                out.write ("Here we go again... \n")
    ...
    def _set_config(self, want, have):
        ...
        def _conf_to_cmd(item):
           switcher = {
             "eee enabled": "eee",
             "eee disabled": "no eee",
             "poe enabled": "power inline supply enable",
             "poe disabled": "power inline supply disable",
             "flow-control enabled": "flow-control",
             "flow-control auto": "flow-control",
             "flow-control disabled": "no flow-control"
           }
       
           return switcher.get(item,item)
        ...
        if diff:
            ...
            for item in self.params:
                if diff.get(item):
                    ...
                    of.write("Item: %s\n" % item)
                    of.write("Want: %s\n" % str(want.get(item)))
                    of.write("Real Item: %s\n" % _conf_to_cmd(item))

                    if (item == 'description'):
                      cmd = item + " " + '\"' + str(want.get(item)) + '\"'
                    else:
                      cmd = item + " " + str(want.get(item))
                      cmd = _conf_to_cmd(cmd) 
    ...
    def _clear_config(self, want, have):
        ...
        if interface_type.lower() == "gigabitethernet":
            ...
            if have.get("flow_control") and want.get("flow_control") != have.get("flow_control"):
                remove_command_from_config_list(interface, "flow-control", commands)

            if have.get("eee") and want.get("eee") != have.get("eee"):
                remove_command_from_config_list(interface, "eee", commands)

            if have.get("poe") and want.get("poe") != have.get("poe"):
                remove_command_from_config_list(interface, "power inline supply enable", commands)
