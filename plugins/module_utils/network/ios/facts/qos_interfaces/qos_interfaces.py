...
"""
The ios_qos_interfaces fact class
...
class Qos_InterfacesFacts(object):
    ...
    def populate_facts(self, connection, ansible_facts, data=None):
        ...
        if objs:
            facts["qos_interfaces"] = []
            params = utils.validate_config(
                self.argument_spec, {"config": objs}
            )
            for cfg in params["config"]:
                facts["qos_interfaces"].append(utils.remove_empties(cfg))
    ...
    def render_config(self, spec, conf):
        ...
        """
        of = open ("/tmp/qos_interfaces.log","a")
        ...
        port_priority = utils.parse_conf_arg(conf, "qos port-priority")
        trust_mode = utils.parse_conf_arg(conf, "qos trust-mode")
        if port_priority:
