...
class Connection(NetworkConnectionBase):
    ...
    def receive_paramiko(
        ...
        of = open ("/tmp/receive_paramiko.log", "a")
        ...
        while True:
            if command_prompt_matched:
                try:
                    ...
                    signal.alarm(0)
                    of.write ("Matched => response-%s: %s" % (self._window_count + 1, data))
                    self._log_messages(
                ...
                data = self._ssh_shell.recv(256)
                of.write ("response-%s: %s" % (self._window_count + 1, data))
                self._log_messages(
                    "response-%s: %s" % (self._window_count + 1, data)
                )
            ...
            of.write ("\n\nWINDOW\n\n%s\n\n" % window)
            regex = re.compile(to_bytes(pgstr), re.I)
            match = regex.search(window)

            if match:
                of.write("\n==== Pager ===\n\n") 
                self._last_recv_window = window
                self._ssh_shell.sendall(b' ')
                time.sleep(0.5)
            ...
            if self._find_prompt(window):
                ...
                resp = self._fstrip(self._last_response)
    ...
    def _fstrip(self, data):
        ...
        m = re.compile(br"Press any key to continue (Q to quit).*", re.I)
        data = m.sub(b"",data)
        return data
    ...
    def _handle_prompt(
        ...
        of = open("/tmp/handle_prompt.log","a")
        of.write ("==> %s\n" % resp)
